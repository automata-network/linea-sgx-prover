...
impl TrieNodeUpdateResult {
    pub fn new_node<D>(db: &mut D, n: TrieStorageNode) -> Self
    where
        D: NodeDB<Node = TrieStorageNode>,
    {
        Self::NewRoot(db.staging(n).as_ref().clone().into())
    }
}
...
pub fn add_nodes<S>(store: &mut S, nodes: Vec<TrieNode>)
where
    S: NodeDB<Node = TrieStorageNode>,
{
    for node in nodes {
